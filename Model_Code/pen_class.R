...
# This script is used for finding the best hyperparameters for a 
# penalized logistic regression model using the glmnet package.
library(tidymodels)
library(tidyverse)
library(arrow)
library(future)
...
# Formula
reg_formula <- w ~ h_a_n_n + h_a_n_h  + rpi + rpi_opp + crpi + crpi_opp + 
  ppg + papg_opp + OWE + DWE_opp + o_rate + d_rate_opp + avg_pace + avg_pace_opp

reg_preprocessing_formula <- w ~ h_a_n+ ppg + OWE + rpi + crpi + 
  papg_opp + DWE_opp + rpi_opp + crpi_opp + g + g_opp + o_rate + d_rate_opp + avg_pace + avg_pace_opp

# Recipe
reg_rec <- recipe(reg_preprocessing_formula, data = train) |> 
...
# Model Specification
reg_spec <- logistic_reg(penalty = tune(), mixture = tune()) |>
  set_engine("glmnet") |> 
...
show_best(reg_bayes_res, metric = "brier_class") |> select(-.estimator) 
show_best(reg_bayes_res, metric = "mn_log_loss") |> select(-.estimator)
show_best(reg_bayes_res, metric = "accuracy") |> select(-.estimator)

# Plot the results
autoplot(reg_bayes_res, metric = "brier_class") 
ggsave("Plots/class_reg_bayes_res.png", width = 10, height = 8)

# Select best parameters
reg_best_params <- select_best(reg_bayes_res, metric = "brier_class")
...
final_reg_res |> collect_metrics() |> mutate(mod_type = "class_glmnet_reg") |> 
  write_rds("Data/class_reg_metrics.rds")
...
# Save the model
library(butcher)
write_rds(butcher(final_reg_res), "Models/class_pen_model.rds")
